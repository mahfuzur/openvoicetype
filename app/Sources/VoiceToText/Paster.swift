...
enum Paster {
    ...
    static func paste(_ text: String, html: String? = nil, autoPaste: Bool) -> Bool {
        ...
        pending = (saved, provider, ourChange) // the pasteboard holds its provider weakly
    ...
    /// Runs a pending clipboard restore now (Command Mode is about to read the clipboard): the user's clipboard comes back
    /// first, so it isn't mistaken for, or lost under, what comes next.
    static func settlePending() {
        guard let pending else { return }
        self.pending = nil
        generation += 1 // the scheduled restore becomes a no-op
        let pasteboard = NSPasteboard.general
        guard pasteboard.changeCount == pending.change, !pending.saved.isEmpty else { return }
        restore(pending.saved, to: pasteboard)
    }
    ...
    private static var pending: (saved: Snapshot, provider: PasteProvider, change: Int)?
    ...
    static func restore(_ saved: Snapshot, to pasteboard: NSPasteboard) {
