...
| M4 | Settings window, first-run setup and DMG | 7–9 days | Planned ([plan](plans/M4-app-and-install.md)) | Anyone can install it from a DMG with no Homebrew or Terminal, and set it up in a real window |
...
plan lists ideas for later. Next: M4, a DMG that installs on a new Mac with a first-run setup and a Settings window ([plans/M4-app-and-install.md](plans/M4-app-and-install.md)).
It starts with four spikes: static Whisper/llama.cpp builds, DMG signing on macOS 15, Claude sign-in from the app, and the compressed model.
